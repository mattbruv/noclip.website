...
// Stand-in for the VU1 microprogram at _$GlowStart: every glow primitive is a
// screen-facing gradient strip. Each vertex carries a unit direction plus a
// class bit; the class picks which (radius, color) pair of the glow definition
// the vertex belongs to, which is what produces the radial falloff.
...
void main() {
    ...
    // Class 0 vertices ride radius/color B, class 1 vertices ride A.
    ...
    // The strip is expanded around the center in view space, so it always faces
    // the camera.
