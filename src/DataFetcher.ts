...
class DataFetcherRequest {
  ...
  constructor(
    ...
    if (
      this.options.rangeStart !== undefined &&
      this.options.rangeSize !== undefined
    ) {
  ...
  private resolveError(): boolean {
    ...
    if (
      (allow404 && this.isConsidered404Error()) ||
      this.abortController.signal.aborted
    ) {
      const emptySlice = new ArrayBufferSlice(
        new ArrayBuffer(0),
    ...
    if (this.retriesLeft > 0) {
      ...
      console.error(
        `DataFetcherRequest: Received non-success status code ${status} when fetching file ${this.url}.`,
      );
      this.reject(null);
      this.done();
      return true;
  ...
  public async start() {
    ...
    if (contentLengthStr === null) contentLengthStr = "1";
    ...
    while (true) {
      try {
        ...
        arrayBuffer =
          totalBytesReceived === 0 ? (value!.buffer as ArrayBuffer) : null;
    ...
    if (this.cache !== null) await this.cache.put(this.request, responseClone);
...
class DataFetcherMount {
  ...
  ): Promise<NamedArrayBufferSlice | null> {
...
export class DataFetcher {
  ...
  public async clearCache() {
    if (this.cache !== null) {
      for (const key of await this.cache.keys()) this.cache.delete(key);
    }
  }
  ...
  public abort(): void {
    this.aborted = true;
    for (let i = 0; i < this.requests.length; i++) this.requests[i].abort();
    this.requests = [];
  }
  ...
  private pump(): void {
    for (
      let i = 0;
      i < Math.min(this.requests.length, this.maxParallelRequests);
      i++
    ) {
      if (!this.requests[i].inFlight()) this.requests[i].start();
    }
  }
  ...
  public fetchURL(
    ...
    if (this.aborted)
      throw new Error(
        "Tried to fetch new data while aborted; should not happen",
      );
